...
import {
  ...
  XCircle, RefreshCw, User, MapPin, ShoppingBag, FileText
} from 'lucide-react'
  ...
  const fetchOrders = async () => {
    ...
    const { data: rows, error } = await supabase
      ...
      .select(`
        ...
        driver_name, customer_name, customer_phone, created_at,
        zones ( name ),
                    ...
                    >
                      ...
                      <td className="hidden lg:table-cell px-4 py-3">
                        <input
                          ...
                          onBlur={e => assignDriver(order.id, e.target.value)}
                          placeholder="Assign…"
                          className="text-xs text-[#4a7fa5] bg-transparent border-b border-[#cce7f0] focus:border-[#0097a7] focus:outline-none w-24 pb-0.5"
                        />
                      </td>
                      ...
                      {selected.order_items.map(item => (
                        <div key={item.id} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-lg bg-[#e0f7fa] flex items-center justify-center text-sm">💧</div>
                            <div>
