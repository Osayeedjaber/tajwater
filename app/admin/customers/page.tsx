...
export default function CustomersPage() {
  ...
  const fetchCustomers = async () => {
    ...
    const { data: profiles, error } = await supabase
      .from('profiles')
      .select('id, name, email, phone, delivery_address, zone_id, wallet_balance, created_at')
    ...
    for (const o of (allOrders ?? [])) {
      ...
      if (!agg[o.user_id]) agg[o.user_id] = { count: 0, total: 0 }
  ...
  const filtered = customers.filter(c => {
    ...
    return (
      ...
      (c.zone_id ?? '').toLowerCase().includes(q)
    )
  })
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#4a7fa5]">
                    ...
                    {customer.zone_id && <span className="flex items-center gap-1"><MapPin  className="w-3 h-3" />{customer.zone_id}</span>}
                    <span className="flex items-center gap-1"><ShoppingBag className="w-3 h-3" />{customer.orderCount} orders</span>
            ...
            <>
              ...
              <div className="space-y-4 mt-2">
                ...
                <div className="bg-[#f0f9ff] rounded-2xl p-4 space-y-1.5">
                  ...
                  {selected.zone_id && <p className="text-sm text-[#4a7fa5] flex items-center gap-2"><MapPin  className="w-4 h-4 text-[#0097a7]" />{selected.zone_id}</p>}
