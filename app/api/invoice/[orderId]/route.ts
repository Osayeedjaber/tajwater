...
type OrderItem = { id: string; quantity: number; price: number; products: { name: string } | null }
type OrderData = {
  id: string
  total: number
...
function InvoicePDF({ order, companyInfo }: { order: OrderData; companyInfo: { name: string; address: string; phone: string; email: string; website: string } }) {
  ...
  const statusColor: Record<string, string> = {
    paid: '#16a34a', pending: '#d97706', failed: '#dc2626', refunded: '#dc2626',
  }
  const payStatus = order.payment_status ?? 'pending'

  return (
    React.createElement(Document, { title: invoiceNum },
...
export async function GET(
  ...
  const { data: order, error } = await db
    .from('orders')
    .select(`
      id, total, payment_status, delivery_address, customer_name, customer_phone, created_at,
      zones ( name ),
  ...
  // renderToBuffer requires a Document element — call as function and cast to satisfy types
  const pdfBuffer = await renderToBuffer(
    InvoicePDF({ order: orderData, companyInfo }) as unknown as React.ReactElement<DocumentProps>
  )
